...
function useSafeTextures(sources: string[]): THREE.Texture[] {
	...
	useEffect(() => {
		...
		if (ctx) {
			...
			ctx.fillText('Bharat Heritage', 256, 256);
		}
...
function GalleryScene({
	...
	const imageSources = useMemo(() => normalizedImages.map((img) => img.src), [normalizedImages]);
	const textures = useSafeTextures(imageSources);
	...
	useEffect(() => {
		const handleWindowWheel = (event: WheelEvent) => {
			// Only trigger when wheel event occurs over canvas or gallery container
			const target = event.target as HTMLElement | null;
			if (target && target.closest('.infinite-gallery-container')) {
				handleWheel(event);
			}
		};
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-[#1B1410] p-4 text-[#FFF6E9]">
...
export function InfiniteGallery({
	...
	if (!webglSupported) {
		return (
			<div className={`infinite-gallery-container ${className}`} style={style}>
			...
			<Canvas
				...
				onCreated={({ gl }) => {
					...
					const handleContextLost = (e: Event) => {
						e.preventDefault();
						console.warn('WebGL context lost in 3D Gallery. Re-initializing...');
						setWebglSupported(false);
					};
