...
const ACCENT_COLORS: AccentColor[] = [
  ...
  { top: "bg-[#FF6A2B]", bottom: "bg-[#E85A1F]", text: "text-white" },
  { top: "bg-[#FFB100]", bottom: "bg-[#E8A000]", text: "text-neutral-900" },
...
const FlapCell = React.memo(function FlapCell({
  ...
  const topBg = accent?.top ?? "bg-[#1B1410]";
  const bottomBg = accent?.bottom ?? "bg-[#1B1410]";
  const textColor = accent?.text ?? "text-[#FFF6E9]";

  const flapTopBg = prevAccent?.top ?? "bg-[#2A1F19]";
  const flapTextColor = prevAccent?.text ?? "text-[#FFF6E9]";
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-3xl rounded-xl bg-[#2A1F19] p-2 shadow-[0_0_30px_rgba(255,106,43,0.15)] md:rounded-2xl md:p-4 border border-[#FF6A2B]/20",
        className
      )}
    >
      <div
